...
export_file_prefix = station + '_mlp_'
...
def prediction_model_function(unit_num, activation, learning_rate):
    opt = Adam(learning_rate = learning_rate)
    # Instatntiate model
    pred_model = Sequential()
    # Adding necessary layers
    pred_model.add(Dense(unit_num,
...
target_train_dataframe['mlp_predicted'] = [np.nan for i in range(target_train_dataframe.shape[0])]
target_train_dataframe.iloc[0:, 1] = y_train_PRED.flatten()
...
target_test_dataframe['mlp_predicted'] = [np.nan for i in range(target_test_dataframe.shape[0])]
target_test_dataframe.iloc[0:, 1] = y_test_PRED.flatten()
